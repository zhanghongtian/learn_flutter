...
class Subtitle extends StatefulWidget {
  Subtitle(
    ...
    this.itemExtent = 45,
    this.diameterRatio = 2.0, this.startRollingLyrics = false,
  }):super(key: key);
...
class _SubtitleState extends State<Subtitle> {
  ...
  _computeDelay() {
    ...
    print("currMinutes:"+currList[0]);
    print("currSeconds:"+currList[1]);
    var currMinutes = _str2Int(currList[0]);
    var currSeconds = _str2Int(currList[1]);

    String nextTime = widget.data[_currentIndex + 1].time;
    nextTime = nextTime.replaceAll("[", "").replaceAll("]", "");
    nextTime = nextTime.substring(0,nextTime.indexOf("."));
  ...
  Widget build(BuildContext context) {
    if (widget.data == null || widget.data.length == 0) {
      return Container(child: Text("暂无歌词..."),);
    }
